...
We defined the basic concept of Matchmaker application as *a tool where any user can  describe the collaborative work he or she intend to support, using the MoCA dimensions, so the application return a list of applications that best match the criteria.*
...
Next, in the development phase, we choose to develop the MatchMaker application as a web application. Most of the computer-based tools available today are accessible through the web, so it was a natural decision to select such platform for our app. While the web interface was developed, based on the mock-ups,  two other activities started to take place, the database set up and the creation of a list of applications.

The database structure was defined by the descriptors and the possible values. But we had to investigate for a platform to host it. Initially, we contacted the Technical Service office at Uvic and got a positive answer; they would set-up a MySQl database for the project and would provide us with the data to access it. However, with Uvic services the user access was limited to only those connections made from campus. With respect to this restriction, we agreed we needed access fro all potential user, outside campus, so we rejected that option. After more research, we found Google's platform, which offered us free host and access to all users. This platform was selected to host the database and one researcher was dedicated to its initial set-up and posterior management.
