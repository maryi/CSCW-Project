...
In addition, Colace, De Santo, Moscato & Picariello (2015) suggest using a recommender system to solve this problem. Recommender systems have emerged to aid decision support in an age of abundance.  Common applications of recommender systems include film/book/music recommendations based on interests (Netflix, Amazon, Spotify), contact suggestions based on social networks (Facebook, LinkedIn) and search results based on user profiles (Google). Recommender systems help users find the tools they need based on user's preferences and feedback and are playing an increasingly important role in our lives. As the MoCA framework provides a way of describing collaborative work scenarios, we believe this information could be used to recommend collaborative applications which could be useful in a specific context.  
