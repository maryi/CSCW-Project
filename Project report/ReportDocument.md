...
### 4. Expected results  -NEEDs TO BE UPDATED, In progress - Eli
...
Initially, we tried to classify applications using the moca framework exclusively, however when we started to actually design our website it was clear that some of the MoCA dimensions weren't going to work. for example the number of communities of practice wasn't necessary to recommend a collaborative application so we replaced it with a question asking the user about what domain of study the application would be used in. We expect that this approach of adding some additional dimensions and removing a few dimensions from the MoCA framework will yield a better collaborative application classification system.

When working on the website we made sure to describe the MoCA dimensions using simple sentences to ensure that the website would be easy for users to understand regardless if what background they have. we also expect that although the search function on the website does not return perfect results it will return good enough results that the user will always be able to find a helpful application. We also think that the process of answering all the search questions will get the user thinking about different areas that they need to think about when considering a collaborative application. Lastly on the website we expect that there may be a lack of motivation for users to register new applications into the database.
